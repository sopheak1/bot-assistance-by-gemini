...
class HourlyTickHandler:
    def __init__(self, user_repo: UserSettingsRepository, clock: Clock, notifier: NotifierPort):
        ...
        async with BotSessionLocal() as bot_session:
            ...
            for binding in bindings:
                async with user_session_maker() as user_session:
                    ...
                    quote_provider = SqliteQuoteProvider(bot_session)
                    ...
                    if msg:
                        await self.notifier.send_message(chat_id=msg.chat_id, text=msg.text, parse_mode="Markdown")
            ...
            for binding in bindings:
                if binding.domain_context.value == "WORK":
                    async with user_session_maker() as user_session:
                        ...
                        rollover_uc = RolloverUnfinishedTasksUseCase(task_repo, rollover_svc, uow, self.clock)
                        ...
                        await self.notifier.send_message(chat_id=binding.chat_id, text="🔄 **Midnight Rollover Completed!**\nUnfinished tasks have been moved to today.", parse_mode="Markdown")
