...
async def start_create_task(
    callback: types.CallbackQuery, state: FSMContext, user_session: AsyncSession
) -> None:
    if not isinstance(callback.message, types.Message):
        return
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=p.name, callback_data=f"task:project:{p.id}")]
            for p in projects
        ]
    )
...
async def process_task_description(
    ...
    try:
        ...
        await message.answer(
            "✅ Task successfully added!", reply_markup=get_main_menu(domain_context)
        )
